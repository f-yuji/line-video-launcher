...
logger = setup_logger("script_generator_v6")
...
それをもとに、TikTok / YouTube Shorts向けの8行台本を作ってください。
狙いは30秒前後です。
...
- 2〜7行目で、ズレてる点、理由、具体例、落とし穴、結論を順に組み立てる
- 8行目は必ず「ここ押さえておかないと、詰む。」で締める
- 1本で言いたいことは1つに絞る
- 30秒前後になるよう、6行台本より一段深く説明する
...
def _parse_script_result(raw: str) -> ScriptResult:
    ...
    if len(display_lines) != 8:
        raise ValueError(f"expected 8 display lines, got {len(display_lines)}")
    if len(speech_lines) != 8:
        raise ValueError(f"expected 8 speech lines, got {len(speech_lines)}")
